 ...
 * <p>
 * Implements ChannelMessageHandler, handling the multi-channel parts and passing things through to
 * individual Channels.
 * <p>
 * Implements Writeable to write out all channel state to disk. Implies peer_disconnected() for all
 * peers during write/read (though does not modify this instance, only the instance being
 ...
 * <p>
 * Note that the deserializer is only implemented for (BlockHash, ChannelManager), which tells you
 * the last block hash which was block_connect()ed. You MUST rescan any blocks along the “reorg
 ...
 * <p>
 * Rather than using a plain ChannelManager, it is preferable to use either a
...
public class LdkChannelManager {
  ...
  public LdkChannelManager(final BlockchainClient blockchainClient,
      final LdkChannelRepo ldkChannelRepo, final LdkContext ldk) {
  ...
  private ChannelManager restoreChannels() {
    ...
    try {
      constructor = new ChannelManagerConstructor(
          manager,
          monitors,
          ldk.getUserConfig(),
    ...
    return constructor.channel_manager;
  }

  private ChannelManager load() {
    final Block blockTip = blockchainClient.getBlockTip();
    final byte[] blockHash = Hex.decode(blockTip.getHash());
    final ChannelManagerConstructor constructor = new ChannelManagerConstructor(
        ldk.getNetwork(),
        ldk.getUserConfig(),
        blockHash,
        blockTip.getHeight(),
  ...
  public void start() {
    ...
    if (ldkChannelRepo.countByStatus(LdkChannelStatus.OPEN) < 1) {
      ...
      _channelManager = load();
    } else {
      // restore existing channels
      _channelManager = restoreChannels();
    }
